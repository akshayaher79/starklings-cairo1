...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            assert!(
                get_caller_address() == self.contract_owner.read(),
                "Not owner"
            );
            self.inventory.entry(product).write(
                self.inventory.entry(product).read() + new_stock
            );
        }

        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            self.inventory.entry(product).write(
                self.inventory.entry(product).read() - quantity
            );
        }
